...
async function checkDonations(walletAddress) {
    if (!walletAddress) {
        ...
        updateExtendedInfo([], 0, null); // Обновляем инфо с нулевыми значениями
        return;
    }
    ...
    try {
        const response = await fetch(`https://tonapi.io/v2/blockchain/accounts/${walletAddress}/transactions?limit=1000`, {
            headers: {
                'Authorization': `Bearer ${TONAPI_KEY}` // Используем константу TONAPI_KEY
            }
        });
        if (!response.ok) {
        ...
        let totalDonation = 0;
        const projectTargetWalletAddress = TARGET_WALLET.toLowerCase(); // Используем глобальную константу TARGET_WALLET
            ...
            if (tx.out_msgs) {
                tx.out_msgs.forEach(msg => {
                    if (msg.destination && msg.destination.address.toLowerCase() === projectTargetWalletAddress) {
                        ...
                        if (!isNaN(valueInTon)) { // Добавлена проверка на NaN
